...
export function buildNewDefinitionPrompt(
	...
	return [
		...
		"- Capability-Aware: `suggest` entries must reference skills/sub-agents that ACTUALLY exist in this environment — verify on disk under ~/.pi/agent/skills/, ~/.agents/skills/, <project>/.pi/skills/, and <project>/.agents/skills/ before naming (your own visible skill list is NOT the full set). Most skills are not visible to the driving model by default, so prefer entries of the form `name:<path-to-SKILL.md-or-its-dir>` so the model can read the SKILL.md directly. Bare `name` entries are allowed. Empty arrays are fine.",
...
export function buildRefinePatternPrompt(file: string, from: "project" | "global", direction?: string): string {
	return [
		...
		"- Capability-Aware: `suggest` entries must reference skills/sub-agents that ACTUALLY exist — verify on disk under ~/.pi/agent/skills/, ~/.agents/skills/, <project>/.pi/skills/, and <project>/.agents/skills/ before naming; prefer `name:<path-to-SKILL.md-or-its-dir>` form. Empty arrays are fine.",
