...
export function buildGoalImplPrompt(snapshot: GoalSnapshot, goalId: string, mode: "start" | "resume" | "continue"): string {
	...
	return [
		...
		"- **Batch fire & end turn:** dispatch ALL leaves of a tier in ONE tool-call batch (`background: true`), then END YOUR TURN. Each completion auto-notifies (sub-dispatch triggerTurn) and wakes you with status + output tail — the notifications ARE the wake-ups, so the runtime sends NO continuation while they are in flight. NEVER `sleep N && dispatch({ sessionId })` poll; use `dispatch({ sessionId })` only as diagnostics/recovery (a notification is missing, or a session errored / exited early / produced no output).",
...
export function buildTrackUpdatePrompt(snapshot: GoalSnapshot): string {
	...
	return [
		...
		"Your ONLY job in this turn: reconcile the two Track files so a fresh session can resume from them. Then STOP and report a one-line summary of what you wrote. Do NOT continue the goal, do NOT start or continue implementation, do NOT dispatch agents, and do NOT take next steps — wait for the user's next instruction. (Goal auto-continuation is also suppressed at the runtime for this turn.)",
