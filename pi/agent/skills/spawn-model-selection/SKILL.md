...
description: Shared single-source-of-truth for choosing which model to run a sub-agent on when dispatching via interactive_shell. Referenced by impl-with-spawn and explore-codebase to avoid maintaining the model-priority list in multiple places. Not meant to be invoked standalone.
...
3. **Neither specified** → pick from what is **actually available on this machine**, cheapest-first, good-enough:
   ...
   - **Simple / mechanical tasks** (refactor, add tests, fix typo, commit messages, cleanup):
     ...
     2. `opencode/hy3` — stronger than mimo2.5, currently 8x quota, 256K context
     3. `opencode/mimo-v2.5` — 1M context multimodal ($0.14/$0.28 per 1M)
   ...
   - **Complex / long-context tasks** (multi-file design, large refactor, architecture):
     ...
     3. `opencode/hy3` — stronger than mimo-v2.5, but 256K context limit
   ...
   - **Notes**:
     ...
     - Prefer `mimo-v2.5` for multimodal; use `hy3` (non-multimodal, stronger reasoning) when multimodal capability is not needed.
...
- **Trust the actual `pi --list-models` output**; opencode-go / official deepseek mentioned elsewhere are only "maybe available" examples — **never assume they exist**. When unsure, default to `pi` (the spawn default agent).
