...
- **Goal → Track is a one-way dependency**: activation auto-resets Track for hygiene (`/goal activate` under the command-driven lifecycle — ADR 0005; originally `/goal run`); `/track new` / `/track update` are fully independent and know nothing of Goals (acyclic = still "orthogonal"). Track never appears on the taskmd board.
