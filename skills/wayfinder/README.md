...
- `pi/agent/prompts/wayfinder.md` —（可选，仅 pi 环境）`/wayfinder` 薄入口

---

## 一句话

> **Wayfinder 管「路线怎么定」；实现层管「路线怎么走完」。**
...
| 系统 | 角色 | 数据位置 |
|---|---|---|
| **Personal Wayfinder** | 决策期导航 | `~/.cache/wayfinder/<workspace-id>/tickets/`（仓库外，无需 gitignore） |
| **taskmd** | 本地后端 + 给人看的 Web UI | 同上（CLI/Web 操作） |
| **实现层**（goal 类工作流、spawn 子代理、直接写代码……按当前环境可用者而定） | 实现期进度与交付 | 各自系统自管 |

```text
路清了    →  交给实现层（goal 工作流 / spawn / 直接写代码，由你选）
```

不要把 Wayfinder Ticket 和实现层的 task 混叫「task」。
...
2. 已执行过 `just install-skills`（技能链到 `~/.agents/skills/`）；pi 环境如需 `/wayfinder` 薄入口，另执行 `just install-pi`
3. 在**目标仓库**里使用（按仓库路径隔离 workspace，但数据存全局目录）
...
以上是 skill 的参数面；pi 环境下通过 `/wayfinder` 薄入口触发，其它 agent 环境直接调用 skill 并附参数即可。

数据目录（全局，仓库内零文件、无需 gitignore）：

```text
~/.cache/wayfinder/<workspace-id>/tickets/
# <workspace-id> 由仓库根绝对路径而来：/ 变 -，末尾补 -，如 -Users-ison-dev-dotfiles-
```
...
→ 关掉/完成这张 Map，agent 会**建议你**按当前环境实际可用的路径进入实现侧（capability-aware：goal 类工作流、spawn 子代理、直接写代码等；由**你**决定并执行，agent 不会替你触发）。
...
以上 skill 均按 availability 处理：环境里没有就跳过，用等价能力兜底。
...
1. **Plan, don't do** — 默认不交付生产实现；Wayfinder 会话对生产代码**只读**（只可写 `~/.cache/wayfinder/` 下的 workspace 和一次性原型草稿；写 handoff spec 算规划不算实现）
...
9. **成熟即毕业** — Ticket 一旦"没有决策、只剩怎么实现"，以 **graduation** 方式 `completed`：Resolution 写 handoff 指针，不在 Wayfinder 里实现；建议**用户**另开会话实现，agent 不代为触发实现入口
...
| `../../pi/agent/prompts/wayfinder.md` | 触发（仅 pi） | 薄快捷入口，不重复方法 |
...
- 后端固定为 **taskmd**（显式依赖）
- **不介入实现层**——对实现类系统保持 suggest-only，由用户触发
